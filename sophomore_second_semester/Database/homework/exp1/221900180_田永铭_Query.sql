-- 1. 有多少物种species的描述description中含有单词“this”？
SELECT COUNT(*) AS speciesCount
FROM species
WHERE description LIKE '%this%';

-- 2. 玩家player‘Cook’将与玩家player‘Hughes’作战。显示他们的用户名username和他们各自拥有的Phonemon的总能量。
...
-- 6. 游戏中的每个玩家player具有特定的等级level。以金额降序列出每一特定等级以及该等级的所有玩家在购买上花费的总金额。
...
-- 7. 什么物品item被购买次数最多？如有并列，列出所有购买次数最多的物品。
...
-- 8. 找到可获取的食物的数量，和购买所有种类食物至少各一次的玩家。
...
-- 9. 将距离最近的两个Phonemon之间的欧氏距离称为X。计算相互之间距离为X的Phonemon对的数量。
...
-- 10. 一些玩家player热衷于某种特定类型type的Phonemon。列出捕捉了任一特定类型type中每一物种species至少各一个Phonemon的玩家的名称以及该类型的名称。
