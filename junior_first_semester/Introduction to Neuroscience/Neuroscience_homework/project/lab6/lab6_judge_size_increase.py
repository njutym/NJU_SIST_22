...
# 数据保存路径
filename = f"./data_increase/{name['Your Name']}_results.csv"
dataRecords = []
...
# 创建一个带绝对值排序的列表，并在绝对值相同的情况下随机打乱
ratios.sort(key=lambda x: abs(x))
ratios = [x for _, x in sorted(zip([abs(r) for r in ratios], ratios), key=lambda pair: (pair[0], random.random()))]
