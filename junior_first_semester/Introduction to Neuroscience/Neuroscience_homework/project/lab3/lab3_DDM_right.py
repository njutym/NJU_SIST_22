...
B = 0.12                      # Decision boundary
start_ratio = 0.3             # Start ratio
...
def simulate_ddm(ratio, drift_rate, sigma, B, dt, max_t_steps, time_non_decision):
    # 根据 start_ratio 设置初始 evidence
    evidence = 0 + start_ratio * B  # 初始位置根据 start_ratio 调整
    evidence_store = np.zeros(max_t_steps)
    choice = 0  # 决策: 0表示无决策，1表示右边界，-1表示左边界
    t_step = 0  # 时间步长
    reaction_time = 0   # 反应时间

    while abs(evidence) < B and t_step < max_t_steps:
        dx = drift_rate * ratio * dt + sigma * np.random.randn() * np.sqrt(dt)  # 欧拉-马鲁亚马方法
        evidence += dx
    ...
    reaction_time = t_step * dt + time_non_decision  # 反应时间
    
    if choice == 0:  # 如果没有做出决策，依据累计的证据作出决策
        choice = np.sign(evidence)
    
    return reaction_time, evidence_store, choice  # 返回反应时间、证据序列、最终决策
...
def grid_search():
    ...
    sigma_grid = np.around(np.linspace(0.05, 0.2, 5), decimals=2)
...
def plot_result(ratios, ddm_acc, experiment_acc, ddm_p_right, experiment_p_right, ddm_reaction_time_mean, experiment_reaction_time_mean, best_drift_rate, best_sigma, best_B, best_mse):
    ...
    plt.savefig("lab3_ddm_fitting_results_right.png")  # 保存图像文件，格式为PNG，可以根据需要修改格式
    plt.show()
...
def plot_evidence_accumulation(ratios, drift_rate, sigma, B, dt, max_t_steps, time_non_decision):
    ...
    # Save the figure
    plt.savefig("lab3_ddm_visualization_of_accumulation_right")
    plt.show() 
    

if __name__ == '__main__':
    
    ratios, experiment_reaction_time_mean, experiment_acc, experiment_p_right = load_experiment_data("./results_right/all_statistic.csv")
