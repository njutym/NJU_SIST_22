...
# 数据保存路径
filename = f"./noised_data/{name['Your Name']}_results.csv"
dataRecords = []
...
noiseLeft = visual.Circle(win, radius=0.1, pos=(-0.62, 0), fillColor=(-0.47, -0.34, -0.17), lineColor=(-0.47, -0.34, -0.17))
noiseRight = visual.Circle(win, radius=0.1, pos=(0.62, 0), fillColor=(-0.47, -0.34, -0.17), lineColor=(-0.47, -0.34, -0.17))
...
for i in range(len(ratios)):
    ...
    noiseleft = random.uniform(0.03, 0.06)
    noiseright = random.uniform(0.03, 0.06)
    sizeRight = sizeLeft * (1 + r)
    biger_size = max(sizeLeft, sizeRight)
    circleLeft.radius = sizeLeft
    circleRight.radius = sizeRight
    noiseLeft.radius = biger_size + noiseleft
    noiseRight.radius = biger_size + noiseright
