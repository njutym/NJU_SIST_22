...
def plot_result(ratios, ddm_acc, experiment_acc, ddm_p_right, experiment_p_right, ddm_reaction_time_mean, experiment_reaction_time_mean, best_drift_rate, best_sigma, best_B):
    ...
    plt.savefig("lab1_ddm_fitting_results.png")  # 保存图像文件，格式为PNG，可以根据需要修改格式
    plt.show()

def plot_result_list(ratios, ddm_acc, experiment_acc, ddm_p_right, experiment_p_right, ddm_reaction_time_mean, experiment_reaction_time_mean, best_drift_rate, best_sigma, best_B):
    # 设置图像大小
    plt.figure(figsize=(12, 12))

    num_subjects = len(experiment_acc)
    colors = cm.viridis(np.linspace(0, 0.9, num_subjects))

    # 准确率对比
    plt.subplot(3, 1, 1)
    plt.plot(ratios, ddm_acc, marker='o', label=f'DDM Model (drift_rate={best_drift_rate}, sigma={best_sigma}, B={best_B})', color='blue')
    for i, acc in enumerate(experiment_acc):
        plt.plot(ratios, acc, marker='x', linestyle='--', label=f'Experiment Data (Subject {i+1})', color=colors[i], alpha=0.5)  # 绘制每个实验者的准确率
    plt.title("Accuracy Comparison: DDM vs Experiment")
    plt.xlabel("Ratios")
    plt.ylabel("Accuracy")
    plt.legend()
    ...
    plt.subplot(3, 1, 2)
    plt.plot(ratios, ddm_p_right, marker='o', label=f'DDM Model (drift_rate={best_drift_rate}, sigma={best_sigma}, B={best_B})', color='blue')
    for i, p_right in enumerate(experiment_p_right):
        plt.plot(ratios, p_right, marker='x', linestyle='--', label=f'Experiment Data (Subject {i+1})', color=colors[i], alpha=0.5)  # 绘制每个实验者的正确概率
    plt.title("Psychometric Curve Comparison")
    plt.xlabel("Ratios")
    plt.ylabel("Right Choices")
    plt.legend()
    ...
    plt.subplot(3, 1, 3)
    plt.plot(ratios, ddm_reaction_time_mean, marker='o', label=f'DDM Model (drift_rate={best_drift_rate}, sigma={best_sigma}, B={best_B})', color='blue')
    for i, reaction_time in enumerate(experiment_reaction_time_mean):
        plt.plot(ratios, reaction_time, marker='x', linestyle='--', label=f'Experiment Data (Subject {i+1})', color=colors[i], alpha=0.5)  # 绘制每个实验者的反应时间
    plt.title("Chronometric Curve Comparison")
    plt.xlabel("Ratios")
    plt.ylabel("Reaction Time")
    plt.legend()
    ...
    # 保存图像文件
    plt.savefig("lab1_multiple_subject_results_display.png")
    plt.show()

def load_and_append_data(file_path, ratios_list, reaction_time_mean_list, acc_list, p_right_list):
    """加载实验数据并将其添加到各自的列表中"""
    ratios, reaction_time_mean, acc, p_right = load_experiment_data(file_path)
    ratios_list.append(ratios)
    reaction_time_mean_list.append(reaction_time_mean)
    acc_list.append(acc)
...
if __name__ == '__main__':
    ...
    # 初始化数据列表
    ratios_list = []
    experiment_reaction_time_mean_list = []
    experiment_acc_list = []
    experiment_p_right_list = []

    # 各实验数据文件路径
    experiment_files = [
        "./results/lux_statistic.csv",
        "./results/lsq_statistic.csv",
        "./results/wqh_statistic.csv",
        "./results/tsy_statistic.csv",
        "./results/jlj_statistic.csv"
    ]

    # 加载并存储各实验数据
    for file_path in experiment_files:
        load_and_append_data(file_path, ratios_list, experiment_reaction_time_mean_list, experiment_acc_list, experiment_p_right_list)
    ...
    # DDM 拟合与绘图
    _, ddm_acc, ddm_p_right, ddm_reaction_time_mean = ddm_fitting(best_drift_rate, best_sigma, best_B)
    plot_result_list(
        ratios, ddm_acc, experiment_acc_list, ddm_p_right, experiment_p_right_list,
        ddm_reaction_time_mean, experiment_reaction_time_mean_list,
        best_drift_rate, best_sigma, best_B
    )
