...
def grid_search():
    # Parameter grids
    drift_rate_grid = np.around(np.linspace(2, 3, 5), decimals=2)
    sigma_grid = np.around(np.linspace(0.15, 0.25, 5), decimals=2)
...
def plot_result(ratios, ddm_acc, experiment_acc, ddm_p_right, experiment_p_right, ddm_reaction_time_mean, experiment_reaction_time_mean, best_drift_rate, best_sigma, best_B, best_mse):
    ...
    plt.savefig("lab4_ddm_fitting_results.png")  # 保存图像文件，格式为PNG，可以根据需要修改格式
    plt.show()
...
def plot_evidence_accumulation(ratios, drift_rate, sigma, B, dt, max_t_steps, time_non_decision):
    ...
    # Save the figure
    plt.savefig("lab4_ddm_visualization_of_accumulation")
    plt.show() 
