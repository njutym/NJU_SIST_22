...
def grid_search():
    ...
    sigma_grid = np.around(np.linspace(0.05, 0.25, 5), decimals=2)
...
def plot_result(ratios, ddm_acc, experiment_acc, ddm_p_right, experiment_p_right, ddm_reaction_time_mean, experiment_reaction_time_mean, best_drift_rate, best_sigma, best_B, best_mse):
    ...
    plt.savefig("lab3_ddm_fitting_results_left.png")  # 保存图像文件，格式为PNG，可以根据需要修改格式
    plt.show()
...
def plot_evidence_accumulation(ratios, drift_rate, sigma, B, dt, max_t_steps, time_non_decision):
    ...
    # Save the figure
    plt.savefig("lab3_ddm_visualization_of_accumulation_left")
    plt.show() 
    

if __name__ == '__main__':
    
    ratios, experiment_reaction_time_mean, experiment_acc, experiment_p_right = load_experiment_data("./results_left/all_statistic.csv")
